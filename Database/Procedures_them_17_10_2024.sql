...
--Để tạo một trigger tự động cập nhật trường HanTro trong bảng Phong khi có dữ liệu NgayVao, bạn có thể sử dụng mã SQL sau đây:
CREATE TRIGGER [dbo].[trg_UpdateHanTro]
ON [dbo].[Phong]
AFTER INSERT, UPDATE
AS
BEGIN
    UPDATE Phong
    SET HanTro = DATEADD(YEAR, 2, NgayVao)
    WHERE MaPhong IN (SELECT MaPhong FROM inserted)
      AND HanTro IS NULL; -- Chỉ cập nhật khi HanTro là NULL
END;
GO
---------- cÓ rồi
...
-- Cập nhật trạng thái phòng khi có tài khoản tạo phòng
CREATE TRIGGER [dbo].[trg_UpdatePhongStatus]
ON [dbo].[UserPhong]
AFTER INSERT
AS
BEGIN
    -- Cập nhật trạng thái của phòng thành 1 khi có tài khoản phòng được tạo
    UPDATE Phong
    SET TrangThai = 1
    FROM Phong p
...
--(3)
-- Cập nhật trang thái thành không khi xóa tài khoản
CREATE TRIGGER [dbo].[trg_UpdatePhongStatus_Xoa]
ON [dbo].[UserPhong]
AFTER DELETE
AS
BEGIN
    -- Cập nhật trạng thái của phòng thành 0 khi xóa tài khoản
    UPDATE Phong
    SET TrangThai = 0
