...
BEGIN
    ...
    DECLARE @lastMaPhong varchar(10);
    DECLARE @numberPart int;

    -- Lấy mã phòng cuối cùng
    SELECT @lastMaPhong = MAX(MaPhong) FROM Phong;

    -- Nếu bảng trống, tạo mã phòng đầu tiên
    IF @lastMaPhong IS NULL
    BEGIN
        SET @newMaPhong = 'P01';
    END
    ELSE
    BEGIN
        -- Lấy phần số từ mã phòng cuối cùng và tăng thêm 1
        SET @numberPart = CAST(SUBSTRING(@lastMaPhong, 2, LEN(@lastMaPhong) - 1) AS int) + 1;

        -- Tạo mã phòng mới với định dạng 'P' + số
        SET @newMaPhong = 'P' + RIGHT('0' + CAST(@numberPart AS varchar(2)), 2);
    END

    -- Trả về mã phòng mới
    SELECT @newMaPhong AS MaPhong;
END;
GO
